...
class OAuth2Component:
    def __init__(self, client_id=None, client_secret=None, authroize_endpoint=None, token_endpoint=None,
                 refresh_token_endpoint=None, revoke_token_endpoint=None, client=None, *, authorize_endpoint=None):
    ...
    async def authorize_button(self, name, redirect_uri, scope, height=600, width=400, key=None, pkce=None,
        ...
        authorize_request = await self.client.get_authorization_url(
        ...
        if result:
            ...
            if 'code' in result:
                ...
                result['token'] = await self.client.get_access_token(**args)
    ...
    async def refresh_token(self, token, force=False):
        """
        ...
        if force or token.get('expires_at') and token['expires_at'] < time.time():
            if token.get('refresh_token') is None:
                ...
                token = await self.client.refresh_token(token.get('refresh_token'))
        return token
    ...
    async def revoke_token(self, token, token_type_hint="access_token"):
        ...
        try:
            await self.client.revoke_token(token, token_type_hint)
        except:
